...
async def create_notification(
    ...
    try:
        ...
        if not await should_send_notification(db, user_id, notification_type):
            logger.debug(f"Skipping notification {notification_type} for user {user_id} (preferences disabled)")
            return None
        ...
        logger.info(f"Created notification {notification.id} of type {notification_type} for user {user_id}")
        return notification
...
async def notify_artifact_created(
    ...
    result = await db.execute(
        select(User).where(
            or_(User.role == "curator", User.role == "viewer"),
            User.is_active == True
        )
    )
...
async def notify_artifact_status_changed(
    ...
    message = f"Статус артефакта '{artifact_title}' изменен: {old_status_name} → {new_status_name}"
...
async def notify_user_created(
    ...
    message = f"Зарегистрирован новый пользователь: {new_user_name} (роль: {new_user_role})"
