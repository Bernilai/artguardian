...
async def get_all_tickets(
    ...
    """Get tickets with filters, sorting, pagination (restorer/curator/admin)."""
    ...
    if sortBy == "priority":
        ...
        if sort_dir == "asc":
            query = query.order_by(
                priority_order.asc(), Ticket.created_at.desc(), Ticket.id.asc()
            )
        else:
            query = query.order_by(
                priority_order.desc(), Ticket.created_at.desc(), Ticket.id.asc()
            )
    else:
...
async def create_ticket(
    ...
    if ticket_data.assigned_to_id:
        ...
        if assigned_user.role != "restorer":
            logger.warning(
                f"User {assigned_user.id} is not a restorer, but assigned to ticket"
            )
    ...
    if artifact.status != "under_restoration":
        ...
        logger.info(
            f"Updated artifact {artifact.id} status to 'requires_attention' "
...
async def update_ticket(
        ...
        if ticket.artifact_id:
            ...
            if artifact:
                if new_status == "in_progress":
                    ...
                    logger.info(
                        f"Updated artifact {artifact.id} status to "
                        f"'under_restoration' (ticket {ticket.id} in progress)"
                    )
                elif new_status == "completed":
                    # Ticket completed → artifact has no defects
                    artifact.status = "no_defects"
                    logger.info(
                        f"Updated artifact {artifact.id} status to 'no_defects' "
                        f"(ticket {ticket.id} completed)"
                    )
                elif new_status == "open" and old_status in [
                    "in_progress",
                    "completed",
                ]:
                    ...
                    logger.info(
                        f"Updated artifact {artifact.id} status to "
                        f"'requires_attention' (ticket {ticket.id} reopened)"
                    )
    ...
    if (
        ticket_data.assigned_to_id is not None
        and ticket.assigned_to_id
        and old_assigned_to_id != ticket.assigned_to_id
    ):
        await notify_ticket_assigned(
            db=db,
            ticket_id=ticket.id,
