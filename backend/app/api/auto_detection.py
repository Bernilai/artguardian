...
async def auto_detect_damage(
    ...
    if not is_artdet_available():
        raise HTTPException(
            ...
            detail=(
                "Automatic detection service is not available. "
                "Please use manual detection."
            ),
        )
    ...
    try:
        ...
        if image_path.startswith("artifacts/"):
            ...
            try:
                ...
                with tempfile.NamedTemporaryFile(
                    ...
                    object_data = minio_service.get_object(
                        bucket_name=minio_service.bucket_name, object_name=image_path
                    )
                ...
                detection_results = artdet_service.detect_damage(
                    image_path=tmp_path, min_confidence=min_confidence
                )
            finally:
                ...
                if tmp_path and os.path.exists(tmp_path):
                    try:
                        ...
                        logger.warning(
                            f"Could not delete temporary file {tmp_path}: "
                            f"{cleanup_error}"
                        )
        else:
            ...
            detection_results = artdet_service.detect_damage(
                image_path=image_path, min_confidence=min_confidence
            )
        ...
        if detection_results["detected"]:
            for i, damage_area in enumerate(detection_results["damage_areas"]):
                detection = Detection(
                    ...
                    # Special type for auto-detection
                    detection_type="auto_detected_damage",
                    ...
                    area_percent=detection_results["damage_percentage"]
                    / detection_results["damage_count"],
                    is_critical=detection_results["damage_percentage"]
                    > 10.0,  # >10% is critical
            ...
            if create_tickets and detection_results["damage_percentage"] > 5.0:
                ...
                ticket_title = (
                    f"Автоматически обнаружено повреждение: "
                    f"{detection_results['damage_percentage']:.1f}%"
                )
                ticket_description = (
                    f"Автоматический анализ выявил {detection_results['damage_count']} "
                    f"областей повреждения общей площадью "
                    f"{detection_results['damage_percentage']:.1f}% изображения."
                )

                # Note: create_ticket is not called here due to circular imports
                ...
                ticket = Ticket(
                    ...
                    priority="high"
                    if detection_results["damage_percentage"] > 10.0
                    else "medium",
                ...
                await create_notification(
                    ...
                    message=(
                        f"Тикет '{ticket.title}' создан на основе автоматического "
                        f"анализа артефакта '{artifact.title}'."
                    ),
        ...
        return {
            ...
            "ticket_created": create_tickets
            and detection_results["damage_percentage"] > 5.0,
        ...
        raise HTTPException(
            ...
            detail=(
                "Не удалось выполнить автоматический анализ изображения. "
                "Пожалуйста, используйте ручное обнаружение повреждений."
            ),
        )
...
async def get_auto_detection_status(
    ...
    return {
        ...
        "message": "Automatic detection is available"
        if is_artdet_available()
