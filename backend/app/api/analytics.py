...
from app.dependencies import require_curator_or_admin
...
async def get_analytics_overview(
    ...
    status_counts_result = await db.execute(
        select(Artifact.status, func.count(Artifact.id).label("count")).group_by(
            Artifact.status
        )
    )
    ...
    collection_counts_result = await db.execute(
        select(Artifact.collection, func.count(Artifact.id).label("count"))
        .group_by(Artifact.collection)
    ...
    ticket_status_counts_result = await db.execute(
        select(Ticket.status, func.count(Ticket.id).label("count")).group_by(
            Ticket.status
        )
    )
    ticket_status_counts = {
        row.status: row.count for row in ticket_status_counts_result.all()
    }
    ...
    ticket_priority_counts = {
        row.priority: row.count for row in ticket_priority_counts_result.all()
    }
    ...
    critical_detections_result = await db.execute(
        select(func.count(Detection.id)).where(Detection.is_critical)
    )
...
async def get_analytics_trends(
    ...
    artifacts_created_result = await db.execute(
        ...
        .where(Artifact.created_at >= start_date)
    ...
    tickets_completed_result = await db.execute(
        ...
        .where(and_(Ticket.completed_at >= start_date, Ticket.completed_at.isnot(None)))
        .group_by(func.date(Ticket.completed_at))
        .order_by(func.date(Ticket.completed_at))
    )
    tickets_completed = [
        {"date": str(row.date), "count": row.count}
...
async def get_restoration_analytics(
    ...
    tickets_by_restorer_result = await db.execute(
        select(User.name, func.count(Ticket.id).label("count"))
    ...
    avg_completion_result = await db.execute(
        ...
        ).where(and_(Ticket.completed_at.isnot(None), Ticket.status == "completed"))
    )
    ...
    priority_completion_result = await db.execute(
        select(
            ...
            func.sum(case((Ticket.status == "completed", 1), else_=0)).label(
                "completed"
            ),
    ...
    priority_completion = [
        {
            ...
            "completion_rate": round((row.completed or 0) / row.total * 100, 1)
            if row.total > 0
            else 0,
        }
    ...
    recent_completions_result = await db.execute(
        select(func.count(Ticket.id)).where(
            and_(Ticket.completed_at >= thirty_days_ago, Ticket.status == "completed")
        )
    )
