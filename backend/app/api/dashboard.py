...
from app.models import Artifact, Ticket, Detection
...
async def get_dashboard_stats(
    ...
    new_artifacts_result = await db.execute(
        select(func.count(Artifact.id)).where(
            Artifact.created_at >= thirty_days_ago
        )
    )
    ...
    # Format trends
    total_trend = f"+{new_artifacts_count} за месяц" if new_artifacts_count > 0 else "Без изменений"
    attention_trend = f"{new_attention_count} новых" if new_attention_count > 0 else "Без изменений"
    critical_trend = f"+{new_critical_count} за неделю" if new_critical_count > 0 else "Без изменений"
