...
class MinIOService:
    ...
    def __init__(self):
        try:
            ...
            logger.warning(
                "MinIO is not available. Image upload features will not work."
            )
            raise
        ...
        try:
            ...
            if image.mode in ("RGBA", "LA", "P"):
                if image.mode == "P":
                    ...
                    background.paste(
                        image.convert("RGBA"),
                        mask=image.split()[1] if len(image.split()) > 1 else None,
                    )
                    image = background
    ...
    def get_presigned_url(
        self, object_path: str, expires: timedelta = timedelta(days=7)
    ) -> str:
        """
        ...
        try:
            url = self.client.presigned_get_object(
                self.bucket_name, object_path, expires=expires
            )
            return url
        except S3Error as e:
...
def get_minio_service() -> MinIOService:
    ...
    if _minio_service_instance is None:
        try:
            ...
            logger.warning(
                "MinIO features will not be available. Make sure MinIO is running."
            )
            raise
