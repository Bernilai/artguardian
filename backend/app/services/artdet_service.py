...
This service wraps the ArtDet Mask R-CNN model for detecting deterioration in paintings.

The service uses TensorFlow SavedModel format for inference (compatible with TensorFlow 2.16+).
"""
import os
import sys
import logging
...
try:
    ...
    from tensorflow.python.framework import tensor_util  # For converting SymbolicTensor to NumPy array
    import skimage
...
class ArtDetService:
    ...
    def __init__(self, model_path: Optional[str] = None):
        ...
        Args:
            model_path: Path to the TensorFlow SavedModel directory (must contain saved_model.pb)
        """
        ...
        if model_path is None:
            ...
            if model_path is None:
                raise FileNotFoundError(
                    ...
                    "(TensorFlow SavedModel format) and extract it to backend/model/model-maskrcnn-tf/"
                )
        else:
            ...
            if model_path_obj.is_dir() and (model_path_obj / "saved_model.pb").exists():
                ...
                raise ValueError(
                    ...
                    "Only TensorFlow SavedModel format (directory with saved_model.pb) is supported."
                )
    ...
    def _load_model(self):
        ...
        try:
            ...
            if hasattr(self.model, 'signatures') and self.model.signatures:
                ...
                try:
                    if 'serving_default' in available_sigs:
                        ...
                        logger.info(f"Loaded 'serving_default' signature: {type(self.infer_fn)}")
                        ...
                        logger.info(f"Loaded first available signature '{available_sigs[0]}': {type(self.infer_fn)}")
            ...
            if self.infer_fn is not None:
                logger.info(f"ArtDet model loaded successfully using SavedModel signature")
            else:
            ...
            raise RuntimeError(
                f"Failed to load ArtDet model from {self.model_path}. "
                f"Error: {str(e)}"
            ) from e
    ...
    def detect_damage(
        ...
        Returns:
            ...
            - damage_areas: List[Dict] - List of detected damage regions with coordinates
            ...
            try:
                ...
                if hasattr(self, 'infer_fn') and self.infer_fn is not None:
                    ...
                    if hasattr(self.model, 'signatures'):
                        try:
                            available_signatures = list(self.model.signatures.keys()) if self.model.signatures else []
                            ...
                            logger.error(f"Error accessing signatures: {sig_error}", exc_info=True)
                ...
                image_meta = np.array([[image_resized.shape[0], image_resized.shape[1], 3] + [0] * 11], dtype=np.float32)
                ...
                try:
                    ...
                    try:
                        outputs = infer(inputs=input_tensor, inputs_1=inputs_1, inputs_2=inputs_2)
                        ...
                        for key, val in outputs.items():
                            try:
                                if hasattr(val, 'op') and hasattr(val.op, 'type'):
                                    op_type = val.op.type
                                    if 'Error' in op_type or 'InvalidArgument' in str(val.op):
                        ...
                        for key, val in outputs.items():
                            try:
                                if 'SymbolicTensor' in str(type(val)) or ('Tensor' in str(type(val)) and not hasattr(val, 'numpy')):
                                    try:
                        ...
                        if any(isinstance(v, np.ndarray) for v in evaluated_outputs.values()):
                ...
                def convert_output(key):
                    ...
                    is_symbolic = 'SymbolicTensor' in str(type(val)) or ('Tensor' in str(type(val)) and not hasattr(val, 'numpy'))
                    if is_symbolic:
                        ...
                        try:
                            if hasattr(tf.compat.v1, 'Session'):
                                graph = val.graph if hasattr(val, 'graph') else tf.compat.v1.get_default_graph()
                ...
                # Process output_0 - shape (1, 100, 6) with [y1, x1, y2, x2, class_id, score]
                if output_0 is not None and isinstance(output_0, np.ndarray):
                    if len(output_0.shape) == 3 and output_0.shape[-1] == 6:
                        ...
                        rois = output_0_data[:, :4]  # First 4 columns are bbox: (100, 4)
                        class_ids = output_0_data[:, 4].astype(np.int32)  # Column 4 is class_id: (100,)
                ...
                # Process output_1 - class_ids or probabilities (only if not extracted from output_0)
                if class_ids is None and output_1 is not None and isinstance(output_1, np.ndarray):
                    if len(output_1.shape) == 3 and output_1.shape[-1] == 2:
                        # Class probabilities: (1, 1000, 2) -> get argmax, then take top 100
                        class_probs = np.argmax(output_1[0], axis=-1)  # (1000,)
                        # If we have rois, take corresponding class_ids
                        if rois is not None and isinstance(rois, np.ndarray) and len(rois.shape) > 0:
                            num_detections = rois.shape[0] if len(rois.shape) > 1 else len(rois) // 4
                            class_ids = class_probs[:num_detections] if num_detections <= len(class_probs) else class_probs
                        else:
                ...
                if scores is None and output_4 is not None and isinstance(output_4, np.ndarray):
                ...
                if masks is None:
                    masks = np.zeros((image_resized.shape[0], image_resized.shape[1], 0))
                ...
                total_detections = results['scores'].shape[0] if len(results['scores'].shape) > 0 else 0
