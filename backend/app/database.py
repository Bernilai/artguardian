...
if use_postgres and not has_db_url_env:
    ...
    logger.info(f"PostgreSQL connection parameters: user={pg_user}, host={pg_host}, port={pg_port} (from {port_source}), db={pg_db}, password_set={bool(use_password)}")
    ...
    logger.info(f"PostgreSQL async connection configured: {pg_user}@{pg_host}:{pg_port}/{pg_db}")
    ...
    if settings.DATABASE_URL.startswith("postgresql://"):
        ...
        database_url = settings.DATABASE_URL.replace("postgresql+asyncpg://", "postgresql+psycopg://", 1)
    elif settings.DATABASE_URL.startswith("postgresql+psycopg2://"):
        database_url = settings.DATABASE_URL.replace("postgresql+psycopg2://", "postgresql+psycopg://", 1)
    else:
    ...
    logger.warning("SQLite with async SQLAlchemy requires aiosqlite. Using sync SQLite for now.")
...
if not is_pgbouncer_connection:
    ...
    logger.info("Using PgBouncer connection - disabling prepared statements and unsupported 'options' parameter")
